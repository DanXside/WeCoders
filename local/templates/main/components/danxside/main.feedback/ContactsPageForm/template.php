...
<div class="col-md-8">
	...
	<div class="row">
		<form id="contact-form" class="default-form" action="<?=POST_FORM_ACTION_URI?>" method="POST">
			...
			<div class="col-md-4 col-sm-6">
				<input name="user_email" type="email" value="<?= $arResult['AUTHOR_EMAIL'] ?>" placeholder="<?= Loc::getMessage('MFT_EMAIL') ?><?= (empty($arParams['REQUIRED_FIELDS'])) || in_array("EMAIL", $arParams['REQUIRED_FIELDS']) ? '*' : ''; ?>"/>
			</div>
			...
			<div class="col-md-12 col-sm-12">
				<textarea name="MESSAGE" cols="30" rows="10" placeholder="<?= Loc::getMessage('MFT_MESSAGE') ?><?= (empty($arParams['REQUIRED_FIELDS'])) || in_array("MESSAGE", $arParams['REQUIRED_FIELDS']) ? '*' : ''; ?>"></textarea>
